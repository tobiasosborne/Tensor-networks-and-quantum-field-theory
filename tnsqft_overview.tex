...
\widetext
\appendix
\section{Parent hamiltonians for the Lifschitz state via the generalised Metropolis algorithm}\label{app:metro}
...
\end{equation}
whose matrix elements are, explicitly,  
\begin{equation}
	K_{xy} \equiv  \min\{e^{-\frac12 (E(x)-E(y))}, e^{\frac12 (E(x)-E(y))}\} \langle x|F|y\rangle.
\end{equation}
Thus we have 
\begin{equation}
	\widehat{K} \equiv  \sum_{x,y=1}^N K_{xy}|y\rangle \langle x|
\end{equation}
...
\begin{equation}
	\min\{e^{-\frac12 (E(x)-E(y))}, e^{\frac12 (E(x)-E(y))}\} = \begin{cases}
		1 &\quad x = y \\
		e^{-\frac12 (E(y)-E(x))} &\quad x < y \\
		e^{-\frac12 (E(x)-E(y))} &\quad x > y.
	\end{cases}
\end{equation}


When we act on $|\Phi\rangle$ with $\widehat{K}$ we obtain
\begin{equation}
	\begin{split}
		\widehat{K}|\Phi\rangle &= \sum_{x,y=1}^N e^{-\frac12E(x)}\min\{e^{-\frac12 (E(x)-E(y))}, e^{\frac12 (E(x)-E(y))}\} \langle x|F|y\rangle |y\rangle \\
		&= \sum_{x,y=1}^N \min\{e^{-E(x)+E(y)}, 1\} \langle x|F|y\rangle e^{-\frac12E(y)}|y\rangle
	\end{split}
\end{equation}
